...
INT_PTR MainDlg::OnLoadProfile( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    std::wstring path;
#ifdef USE64
    if (OpenSaveDialog( L"Loader profiles (*.xpr64)\0*.xpr64\0", 1, path ))
#else
    if (OpenSaveDialog( L"Loader profiles (*.xpr)\0*.xpr\0", 1, path ))
#endif
    {
        // Reset loaded images
        _profileMgr.config().images.clear();
...
INT_PTR MainDlg::OnSaveProfile( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
    std::wstring path;
#ifdef USE64
    if (OpenSaveDialog( L"Loader profiles (*.xpr64)\0*.xpr64\0", 1, path, true, L"xpr64" ))
#else
    if (OpenSaveDialog( L"Loader profiles (*.xpr)\0*.xpr\0", 1, path, true, L"xpr" ))
#endif
    {
        SaveConfig( path );
        _status.SetText( 0, blackbone::Utils::StripPath( path ) );
